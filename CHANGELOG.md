...
- **End-to-end threat model review.** STRIDE pass over the running
  v0.4.15 surface — 0 critical findings. `SECURITY.md` refreshed
  to reflect what's covered (JWT validation hardening, transport
  security, audit cadence) without exposing the internal
  defensive details that don't help deployers. Deeper
  default-posture changes (binding, rebind protection, metrics
  endpoint gating) are tracked privately in the pre-1.0 hardening
  milestone — they're breaking-by-default and will land in a
