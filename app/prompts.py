# app/prompts.py
"""Curated MCP Prompts — workflows for farmacia, hospital, industria, and
non-specialist users.

A prompt is a server-defined workflow template that the user invokes
explicitly from any MCP-compliant client (Claude Desktop, Continue,
Cursor, Zed, …). The body returned by each function below is sent to
the LLM as a user message; it instructs the LLM **which mcp-aemps tools
to call, in what order, and how to format the output**.

Why these eight prompts:

* They cover four user segments (community pharmacy, hospital pharmacy,
  pharma industry, non-specialist) without overlapping each other.
* They all leverage the rich payload that ``obtener_medicamento`` and
  ``obtener_presentacion`` already return — ``docs[]`` (FT, Prospecto,
  IPE/IPT, RMP), ``fotos[]`` (caja + forma farmacéutica), the
  ``materialesInf`` flag, and the ``materiales/:nregistro`` videos for
  inhalers / autoinjectors / pen devices — instead of treating CIMA as
  a thin field lookup.
* Every prompt that touches patient-facing context closes with an
  explicit "no es consejo médico" disclaimer. The MCP server is a
  regulatory data source, never a clinical decision support system
  (MDR 2017/745 line — see CLAUDE.md compliance notes).

Adding a new prompt: implement one async function below decorated with
``@register_prompt``, then it appears on the stdio MCP transport
automatically. (HTTP transport via fastapi-mcp 0.4.x does not expose a
prompts surface — see CHANGELOG v0.2.6 for the limitation; will be
addressed in v0.3 along with MCP Resources.)
"""
